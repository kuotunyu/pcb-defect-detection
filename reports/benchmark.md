# Legacy benchmark — not release evidence

> The rows below predate the paired protocol and new provenance schema. They lack the complete
> checkpoint/export/manifest/environment chain required for current claims and are permanently
> superseded by [`benchmark_l4.json`](benchmark_l4.json). They are preserved only as exploratory
> history and must not be used as current release evidence.
...
- **本機 CPU（全執行緒）vs T4 PyTorch FP32**：本機 CPU 反而快 +26.2%。這不是公平的硬體成本對比（筆電 CPU vs 資料中心 GPU），也不支持任何 hosted deployment 選型；它只描述當時的 exploratory batch=1 量測。
...
- 「2-thread proxy」不是 hosted deployment 實測，不支持任何 Space 或 production latency claim。

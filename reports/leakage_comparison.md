# Legacy exploratory evidence — not a paired leakage estimate

> These runs used different test images and test sizes. The 12.1-point observation is retained as
> historical split sensitivity only; it must not be presented as a causal leakage effect or a
> current model result. See `reports/claims.yaml` and the frozen paired protocol.
